...
public class LongBitSet {
    ...
    /**
     * Map from a value stored in high bits of a long index to a bit set mapped to the lower bits of an index. Bit sets
     * size should be balanced - not to long (otherwise setting a single bit may waste megabytes of memory) but not too
     * short (otherwise this map will get too big). Update value of {@code VALUE_BITS} for your needs. In most cases it
     * is ok to keep 1M - 64M values in a bit set, so each bit set will occupy 128Kb - 8Mb.
     */
    private final Map<Long, BitSet> m_sets = new HashMap<Long, BitSet>(20);
    ...
    // *
    // * @param proc
    // * Procedure to call. If it returns {@code false}, then iteration will stop at once
    // */
