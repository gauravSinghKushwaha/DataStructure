package com.nimbuzz.collection;
...
public class ConcurrentSkipListMapExample {
    ...
    public static void main(final String[] args) {
        ...
        final NavigableSet navigableKeySet = concurrentSkipListMap.keySet();

        System.out.println("The keys associated with this map are ");

        for (final Iterator iterator = navigableKeySet.iterator(); iterator.hasNext();)
        ...
        final NavigableSet navigableSubKeySet = subMap.keySet();

        System.out.println("The keys associated with the submap keys from 1111 to 3333 are");

        for (final Iterator subMapIterator = navigableSubKeySet.iterator(); subMapIterator.hasNext();)
        ...
        final NavigableSet navigableHeadMapKeySet = headerMap.keySet();

        for (final Iterator headMapIterator = navigableHeadMapKeySet.iterator(); headMapIterator.hasNext();)
        ...
        final NavigableSet navigableTailMapKeySet = tailMap.keySet();

        for (final Iterator tailMapIterator = navigableTailMapKeySet.iterator(); tailMapIterator.hasNext();)
